...
import math
from models.rota_quant_layer_v2 import *
...
class VGG_rotaquant2(nn.Module):
    def __init__(self, vgg_name):
        super(VGG_rotaquant2, self).__init__()
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                layers += [RotaQuantConv2d2(in_channels, 16, kernel_size=3, padding=1),
                           nn.ReLU(inplace=True)]
                in_channels = 16
            else:
                layers += [RotaQuantConv2d2(in_channels, x, kernel_size=3, padding=1),
                           nn.BatchNorm2d(x),
                           nn.ReLU(inplace=True)]
                in_channels = x
...
def VGG16_rotaqv2(**kwargs):
    model = VGG_rotaquant2(vgg_name = 'VGG16_quant', **kwargs)
    return model

def VGG16_rotaq2v2(**kwargs):
    model = VGG_rotaquant2(vgg_name = 'VGG16_quant2', **kwargs)
    return model
