
import torch
import torch.nn as nn
import math
from models.rota_quant_layer import *


## X marks the layer being modified to fit our needs
cfg = {
    'VGG16_quant': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 256, 8, 'X', 'M'],
    'VGG16_quant2': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 256, 16, 'Y', 'M']
    # 'VGG16_quant': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 8, 'X', 'M']
    #'VGG16_quant': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M'] # original,
    # 'Custom': [64, 64, 'M', 128, 128,'M', 256, 256, 256, 'M', 128, 128, 128, 'M', 64, 64, 64, "M", 8, 8,'M']
}


class VGG_rotaquant(nn.Module):
    def __init__(self, vgg_name):
        super(VGG_rotaquant, self).__init__()
        self.features = self._make_layers(cfg[vgg_name])
        if vgg_name == "VGG16_quant2":
            self.classifier = nn.Linear(64, 10)
        else:
            self.classifier = nn.Linear(32, 10)

    def forward(self, x):
        out = self.features(x)
        out = out.view(out.size(0), -1)
        # print(out.shape)
        out = self.classifier(out)
        # print(out.shape)
        return out

    def _make_layers(self, cfg):
        layers = []
        in_channels = 3
        for x in cfg:
            if x == 'M':
                layers += [nn.MaxPool2d(kernel_size=2, stride=2)]
            # elif x == 'F':  # This is for the 1st layer
            #     layers += [nn.Conv2d(in_channels, 64, kernel_size=3, padding=1, bias=False),
            #                nn.ReLU(inplace=True)]
            #     in_channels = 64
            elif x == 'X':
                layers += [RotaQuantConv2d(in_channels, 8, kernel_size=3, padding=1),
                           nn.ReLU(inplace=True)]
                in_channels = 8
            elif x == 'Y':
                layers += [RotaQuantConv2d(in_channels, 16, kernel_size=3, padding=1),
                           nn.ReLU(inplace=True)]
                in_channels = 16
            else:
                layers += [RotaQuantConv2d(in_channels, x, kernel_size=3, padding=1),
                           nn.BatchNorm2d(x),
                           nn.ReLU(inplace=True)]
                in_channels = x
        layers += [nn.AvgPool2d(kernel_size=1, stride=1)]
        return nn.Sequential(*layers)

    def show_params(self):
        for m in self.modules():
            if isinstance(m, QuantConv2d):
                m.show_params()

    

def VGG16_rotaq(**kwargs):
    model = VGG_rotaquant(vgg_name = 'VGG16_quant', **kwargs)
    return model

def VGG16_rotaq2(**kwargs):
    model = VGG_rotaquant(vgg_name = 'VGG16_quant2', **kwargs)
    return model

# def Custom_quant(**kwargs):
#     model = VGG_quant(vgg_name = 'Custom', **kwargs)
#     return model



